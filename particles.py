            ...
            if self.yAddress == 0 or self.yAddress == yCells - 1:  # this is when we are in the borders
                ...
                try:
                    ...
                    # if the right square below is empty
                        ...
                        self._go_right(grid, self.xAddress, self.yAddress)
                    # if all the lower squares are full go left if possible
                    elif (grid[self.xAddress][self.yAddress + 1].state == False):
                        self._go_right_right(grid, self.xAddress, self.yAddress)
                    # if all the lower squares are full go right if possible
                    elif (grid[self.xAddress][self.yAddress - 1].state == False):
                        self._go_left_left(grid, self.xAddress, self.yAddress)
                except:
                    pass
